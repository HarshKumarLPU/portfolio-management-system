    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">No portfolio found for @{username}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        {activeTab === 'about' && (
          <div className="bg-white rounded-2xl shadow-sm p-8">
            <h2 className="text-xl font-bold text-gray-800 mb-4">About {user?.fullName}</h2>
            <p className="text-gray-600 leading-relaxed">{portfolio?.about || 'No bio added yet.'}</p>
            ...
            {portfolio?.projects?.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {portfolio.projects.map((project, index) => {
                  ...
                  return (
                    <div key={project._id} className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition hover:-translate-y-1 duration-200">
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {project.tags?.map((tag) => (
                            <span key={tag} className="bg-indigo-50 text-indigo-600 text-xs font-medium px-3 py-1 rounded-full">{tag}</span>
                          ))}
                        </div>
                        <div className="flex gap-3">
                          {project.github && (
                            <a href={project.github} target="_blank" rel="noreferrer" className="text-xs font-semibold text-gray-600 border border-gray-300 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition">
                              🐙 GitHub
                            </a>
                          )}
                          {project.live && (
              ...
              <p className="text-center text-gray-400 py-12 bg-white rounded-2xl shadow-sm">No projects added yet.</p>
            )}
          </div>
        )}
            ...
            {portfolio?.experience?.length > 0 ? (
              <div className="space-y-6">
                {portfolio.experience.map((exp, index) => (
                  <div key={exp._id} className="bg-white rounded-2xl shadow-sm p-6 flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-4 h-4 rounded-full mt-1 flex-shrink-0 ${exp.current ? 'bg-green-500' : 'bg-gray-300'}`} />
                      {index !== portfolio.experience.length - 1 && <div className="w-0.5 bg-gray-200 flex-1 mt-2" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-lg font-bold text-gray-800">{exp.role}</h3>
                        {exp.current && <span className="bg-green-100 text-green-600 text-xs font-semibold px-2 py-0.5 rounded-full">Current</span>}
                      </div>
                      <p className="text-indigo-600 font-medium text-sm">{exp.company}</p>
                      <p className="text-gray-400 text-xs mt-0.5">🗓️ {exp.duration}</p>
              ...
              <p className="text-center text-gray-400 py-12 bg-white rounded-2xl shadow-sm">No experience added yet.</p>
            )}
          </div>
        )}
            ...
            {portfolio?.education?.length > 0 ? (
              <div className="space-y-6">
                {portfolio.education.map((edu) => (
                  <div key={edu._id} className="bg-white rounded-2xl shadow-sm p-6">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">🎓</div>
                      <div>
                        <h3 className="text-lg font-bold text-gray-800">{edu.degree}</h3>
                        <p className="text-indigo-600 font-medium text-sm">{edu.school}</p>
                        <p className="text-gray-400 text-xs mt-0.5">🗓️ {edu.duration}</p>
                        <p className="text-gray-600 text-sm mt-2 font-medium">{edu.grade}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <p className="text-center text-gray-400 py-12 bg-white rounded-2xl shadow-sm">No education added yet.</p>
            )}
          </div>
        )}
